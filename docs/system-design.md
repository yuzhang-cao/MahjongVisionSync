...
赛事数据分析模块保留现有 `Engine.swift` 服务广东/四川单机流程，并通过独立规则档案实现 MCR 与 M.League 风格 Riichi。该模块不直接处理图像识别，只接收已经融合后的完整牌局状态、规则和计分上下文。输出指标用于工作人员核验、转播展示、观众理解和赛后复盘，不向参赛选手提供实时决策建议。
...
摸入有效牌概率使用简化公式，并明确作为辅助指标：

```text
摸入有效牌概率 = 剩余有效牌数量 / 剩余未知牌数量
```

个人胜率必须由蒙特卡洛模拟产生：随机补全未知牌，按版本化基线策略模拟后续摸打，统计四家最终获胜份额和流局概率。五项结果归一化总和为 1，并记录模拟次数、随机种子、算法版本和不确定性；它不能由下一摸进张率替代。
...
第五阶段新增 `DrawProbabilityAnalyzer` 和 `EquityAnalyzer`：前者计算剩余有效牌与下一摸进张率，后者以蒙特卡洛模拟计算四家获胜份额和流局概率。
...
本系统优先选择可解释、可演示、可测试的实现路线。第一版使用公开比赛视频切流或固定机位视频，是因为它能先验证多路分区感知、事件化和状态重建闭环；实时部署再扩展 `MultipeerConnectivity`，避免一开始把主要风险压在多设备调试上。复盘数据第一版使用 JSON，是因为事件日志和状态快照更容易被检查、导出和写入论文。下一摸进张率采用剩余有效牌比例模型；个人胜率采用记录随机种子、模拟次数和算法版本的蒙特卡洛模型，使结果可复现、可解释、可测试。
