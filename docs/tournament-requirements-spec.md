...
MCR 牌具规格固定为 144 张：34 种基础牌各 4 张，加春、夏、秋、冬、梅、兰、竹、菊 8 张独立牌面。识别模型必须保留 42 个视觉语义类别，不能把 8 张花牌合并为一个类别。
...
M.League 风格 Riichi 牌具规格固定为 136 张，其中 5 万、5 筒、5 索各有 1 张替换为赤五。识别模型必须区分 34 种基础牌与 3 种赤五外观，共 37 个视觉语义类别；规则计数可以将赤五映射回对应基础 5，但识别事件必须保留赤牌身份。
...
- `mcrTileModel`：面向 MCR/国标比赛或中文麻将牌面风格，输出 42 个视觉语义类别。
- `riichiMLeagueTileModel`：面向 M.League 风格 Riichi 牌面、赤宝牌和日麻视觉风格，输出 37 个视觉语义类别。
...
Equity 第一版必须使用蒙特卡洛模拟：根据当前牌局状态随机补全未知牌，并按带版本号的基线摸打策略模拟后续过程。它是可解释的估计值，不宣称预测真实选手决策，但不能退化为下一摸进张率或任意趋势指数。
...
- `SimulationConfig`：模拟次数、随机种子、基线摸打策略和算法版本。

最小输出：

- 东、南、西、北四家的最终获胜份额。
- 流局概率。
- 四家获胜份额与流局概率之和必须归一化为 1，允许浮点误差；规则允许多家和牌时，一次模拟的 1 份结果按 `RulesProfile` 规定拆分。
- 模拟次数、随机种子、指标版本和计算方法标识。
- 置信度或不确定性说明。
...
`drawEffectiveProbability` 继续作为独立辅助指标展示，不参与上述五项结果的归一化，也不得命名为个人胜率。
