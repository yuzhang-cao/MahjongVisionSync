...
现有 `Engine.swift` 只能继续服务广东/四川单机听牌和有效牌分析。赛事模式应通过独立 `RulesProfile` 接入 MCR 与 M.League 风格 Riichi，并新增面向赛事工作人员和观众的 `EquityAnalyzer`。这些指标用于转播展示和复盘说明，不向参赛选手提供实时决策建议。

建议新增两个职责分离的分析器：

- `DrawProbabilityAnalyzer` 输出听牌列表、有效牌数量、剩余有效牌数量和下一摸进张率。
- `EquityAnalyzer` 输入四家牌局状态、规则档案、计分上下文和模拟配置，通过蒙特卡洛模拟输出四家最终获胜份额与流局概率。
- 四家获胜份额与流局概率必须归一化为 1，并保存模拟次数、随机种子、算法版本和不确定性说明。
- 两类指标都保存状态变化快照，用于趋势图和复盘。

下一摸进张率仍使用可解释的简化公式，但它只是辅助指标：

```text
摸入有效牌概率 = 剩余有效牌数量 / 剩余未知牌数量
```

个人胜率不能由该公式替代；第一版赛事闭环必须包含上述蒙特卡洛 equity。
...
5. 主控端通过独立规则档案分析 MCR 与 M.League 风格 Riichi，并展示听牌/进张辅助指标以及四家获胜份额加流局率等于 100% 的蒙特卡洛 equity。
...
完成赛事侧五点与训练侧四点后，项目就从“单机手牌分析基础能力”升级为“多路视频赛事数据原型系统”，能够支撑毕设核心演示，也为后续赛事合作展示打下基础。
